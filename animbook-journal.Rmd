...
The interactive chart featured in the New York Times article (Figure \@ref(fig:nyt)) unveils the issue of income disparities between black and white children who were raised in families with comparable income according to the @race. This visualization reveals that, compared to white children, black children are more likely to drop down to the lower-income group, given that they both grew up in wealthy families.
  ...
  There was an implementation of the New York Times animation in R (@r) by @Hvitfeldt_2018. This article demonstrates the path generation for each point and how to map them to the animation. Taking inspiration from this example, \CRANpkg{animbook} expands upon the visualization by incorporating additional elements, particularly the proportional shaded area, and introduces new visualizations inspired by the New York Times animation.
  
  As mentioned about the proportional shaded area, a similar static visualization of the animation is known as the Sankey plot. Sankey plots are flow diagrams that show the movement/change from one state to another, with the width of each arrow representing the proportional changes between states. R packages capable of generating a Sankey plot type include  \CRANpkg{ggalluvial} and \CRANpkg{ggparallel}, both are extensions from the \CRANpkg{ggplot2} {@ggplot2}. The main difference between the Sankey plot visualization and the animation featured in the New York Times lies in their methods of representing the proportion of changes between states. Sankey plots utilize the width of connecting lines, while the New York Times animation employs the density and color of points accompanied by counters. Combining the differences between these two features will be shown in the Visualization design section.
...
```
  ...
  In the animation, the use of proportional shaded areas serves to enhances the quick comprehension of proportion information, effectively displaying the distribution within each group. This method replaces the coutners seen in the New York Times animation, alleviating information overload for users and reducing the computation required. To represents sub-groups, the plot utilizes color to distinguish differences among observation.

  However, discerning the movement pattern or proportion of sub-groups can be challenging when the visualization lacks a sufficient number of data points. Addressing this, observation interpolation is employed by calculating the sub-group proportion and multiplying it by the desired number of generated points. This approach ensures a more detailed representation of movement patterns, particularly in scenarios where a limited number of data points may impede visual clarity.
  
  The issue with this type of animation in the New York Times lies in point clustering, particularly when all points start simultaneously. This clustering poses a challenge in distinguishing and tracking individual observations. In the context of time series data (e.g., year), the observations in the animation enter the visualization at different times. It is important to note that this does not necessarily reflect the real-time occurrence of observations. The neglect of the time-dependent nature of observations is a deliberate choice made for visualization purposes, aiming enhance the clarity of the presented information.
...
```
  ...
  The argument for the `anim_prep()` and `anim_prep_cat()` function are listed in the Table `r knitr::asis_output(ifelse(knitr::is_latex_output(), '\\ref{tab:tbl-num-la}','\\@ref(tab:tbl-num)'))` and `r knitr::asis_output(ifelse(knitr::is_latex_output(), '\\ref{tab:tbl-cat-la}','\\@ref(tab:tbl-cat)'))` respectively. The `data`, `id`, `values`, and `time` arguments are required for both functions.
...
```{r catchange-gif, out.extra="class = 'gif'", fig.cap = "Animate visualization using example data. All of the X observations stay within the same group, while some of the Y observations change from group A to group E.", eval=knitr::is_html_output()}
...
The Osiris data presented in this section was acquired from Bureau van Dijk (@bvd), compiling information on a total of 122,318 companies. It was subsequently a subset for the 2021 report, consisting of 1000 companies with 94 variables. The dataset included in the package focuses on 790 companies, spanning from 2006 to 2018. The variables in this dataset include year, ID, country, sales, and japan.
  ...
  @oecd_report reported that companies in the United States have a faster metabolic rate relative to Japanese companies. This means that US companies tends to move up and down performance metrics more frequently than Japanese companies. This can be examine using the wallaby and kangaroo plots. The wallaby plot allows the study of entry and exit behavior. The kangaroo plot allows studying the full movement of companies over time.
...
Beginning with inspiration drawn from the New York Times article, this packages provides tools to facilitate the communication of complex data to broader audiences. The paper outlies the anticipated data format for input into the prep function, covering raw, categorized, and animated formats. It delves into the animation concept, persistence of vision, achieved through point interpolation, and the visualizatoin design choices.

The paper further proviedes illustrative examples of software usage through two application to the real world data. Given more time, introducing geatures that allow variation in speeds for each observation could enhance the visualization by introducing an additional dimension. The new animated adn static plots, as demonstrated in the New York Times article, would enable comparisons of demographics at specific points in time. Additionally, the potential re-parameterization of the x-axis from years to ages (e.g., company) could add different details to the visualization.

- positives and negatives about the method
- extensions that would be desirable for the software
