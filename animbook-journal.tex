...
\item
  Prepare the data using the \texttt{anim\_prep} function with the first scaling, which is a ranking method. The variable \texttt{sales} is used as the performance metric. Four quantiles are used as specified by the \texttt{ncat} option, and NAs formed a fifth category labeled as ``Not listed''.
\item
...
From Figure \ref{fig:osiris-figure}, there are clearly more US companies de-listed compared to Japanese companies, and it can be seen that there are no Japanese companies in the Top 25\% in 2006 that were de-listed.
